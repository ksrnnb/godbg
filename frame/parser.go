 ...
 * https://github.com/go-delve/delve/blob/v0.7.0-alpha/dwarf/frame/parser.go
 */
...
func parseCIE(ctx *parseContext) parsefunc {
	...
	// parse augmentation
	ctx.common.Augmentation, _ = decoder.ParseString(buf)

	// parse code alignment factor
	ctx.common.CodeAlignmentFactor, _ = decoder.DecodeULEB128(buf)

	// parse data alignment factor
	ctx.common.DataAlignmentFactor, _ = decoder.DecodeSLEB128(buf)

	// parse return address register
	ctx.common.ReturnAddressRegister, _ = decoder.DecodeULEB128(buf)
