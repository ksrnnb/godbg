 ...
 * https://github.com/go-delve/delve/blob/v0.7.0-alpha/dwarf/frame/table.go
 */
package frame
...
func offset(frame *FrameContext) {
	...
	var (
		reg       = b & low_6_offset
		offset, _ = decoder.DecodeULEB128(frame.buf)
	)
...
func offsetextended(frame *FrameContext) {
	var (
		reg, _    = decoder.DecodeULEB128(frame.buf)
		offset, _ = decoder.DecodeULEB128(frame.buf)
	)
...
func register(frame *FrameContext) {
	reg1, _ := decoder.DecodeULEB128(frame.buf)
	reg2, _ := decoder.DecodeULEB128(frame.buf)
...
func defcfasf(frame *FrameContext) {
	reg, _ := decoder.DecodeULEB128(frame.buf)
	offset, _ := decoder.DecodeSLEB128(frame.buf)
...
func defcfaexpression(frame *FrameContext) {
	var (
		l, _ = decoder.DecodeULEB128(frame.buf)
